/*
    Access control for super-admins, admins and only the candidate that has created the document.
...
export const SA_A_C_Self: Access = ({ req: { user } }) => {
  if (user) {
    if (checkRole('super-admin', user) || checkRole('admin', user)) {
      return true
    } else if (checkRole('candidate', user)) {
      return {
        createdBy: {
          equals: user.id,
        },
